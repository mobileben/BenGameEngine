...
#define INNER_ARC_PUSH \
    ...
    hold.push( *apparent_P, C2);
                        ...
                        if ( i>100) {
                            VASER_DEBUG_OUT("trapped in loop: inc,ig_end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
                            break;
                        }
                    }
                ...
                else
                {
                    ...
                    for ( float a=angle1; ; a+=dangle, i++)
                    {
                        ...
                        if ( i>100) {
                            VASER_DEBUG_OUT("trapped in loop: inc,end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
                            break;
                        }
                    }
                }
            ...
            else //decremental
            {
                if ( ignor_ends)
                {
                    ...
                    for ( float a=angle1-dangle; a>angle2; a-=dangle, i++)
                    {
                        ...
                        if ( i>100) {
                            VASER_DEBUG_OUT("trapped in loop: dec,ig_end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
                            break;
                        }
                    }
                }
...
#undef INNER_ARC_PUSH
                        ...
                        if ( i>100) {
                            VASER_DEBUG_OUT("trapped in loop: dec,end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
                            break;
                        }
                    }
                }
...
#define push_quadf(A0,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10,A11,A12) push_quadf_(__LINE__,A0,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10,A11,A12)
        ...
        {
            if( P0.is_zero()) VASER_DEBUG_OUT("pushed P0 (0,0) at %d\n",line);
            if( P1.is_zero()) VASER_DEBUG_OUT("pushed P1 (0,0) at %d\n",line);
            if( P2.is_zero()) VASER_DEBUG_OUT("pushed P2 (0,0) at %d\n",line);
            if( P3.is_zero()) VASER_DEBUG_OUT("pushed P3 (0,0) at %d\n",line);
        ...
        {	//an iterative implementation
            ...
            if ( N > MAX_ST)
            {
                VASER_DEBUG_OUT("vah_N_knife_cut: max N for current build is %d\n", MAX_ST);
                N = MAX_ST;
            }
                ...
                {	//2nd to 2nd last point
                    ...
                    { //must do intersection
                        ...
                        if ( result3) {
                            ...
                            SL[i].vP=SL[i].T;
                            SL[i].vR=SL[i].R;
                            VASER_DEBUG_OUT( "intersection failed: cos(angle)=%.4f, angle=%.4f(degree)\n", cos_tho, acos(cos_tho)*180/3.14159);
                        }
                    }
