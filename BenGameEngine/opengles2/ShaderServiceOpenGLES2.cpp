...
std::shared_ptr<BGE::ShaderProgram> BGE::ShaderServiceOpenGLES2::createShaderProgram(ShaderProgramId id, std::string name, std::vector<std::shared_ptr<Shader>> shaders, std::vector<std::pair<ShaderVertexAttributeIndex, std::string>> attributes, std::vector<std::pair<ShaderUniformId, std::string>> uniforms, std::function<void(ShaderProgram *program)> firstUseFunction, std::function<void(ShaderProgram *program)> windowMappedDimensionsUpdated, std::function<void(ShaderProgram *program)> shaderChangedSetup)
{
    std::shared_ptr<ShaderProgramOpenGLES2> program = std::dynamic_pointer_cast<ShaderProgramOpenGLES2>(shaderPrograms_[name]);

    if (!program) {
        program = std::make_shared<ShaderProgramOpenGLES2>(id, name, shaders, attributes, uniforms, firstUseFunction, windowMappedDimensionsUpdated, shaderChangedSetup);
