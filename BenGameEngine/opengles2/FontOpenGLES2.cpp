...
std::shared_ptr<BGE::FontOpenGLES2> BGE::FontOpenGLES2::create(std::string name, uint32_t pixelSize) {
    return std::make_shared<FontOpenGLES2>(private_key{}, name, pixelSize);
}

BGE::FontOpenGLES2::FontOpenGLES2(struct private_key const& key, std::string name, uint32_t pixelSize) : BGE::Font(name, pixelSize) {
}

void BGE::FontOpenGLES2::load(std::string filename, uint32_t faceIndex, std::function<void(std::shared_ptr<Font>, std::shared_ptr<BGE::Error> error)> callback)  {
    FT_Face face = NULL;
    FT_Error error = 0;
    FT_Library library;
...
void BGE::FontOpenGLES2::drawString(std::string str, std::shared_ptr<TransformComponent> transform, Color &color, FontHorizontalAlignment horizAlignment, FontVerticalAlignment vertAlignment, bool minimum) {
    
    drawString(str, transform->getMatrixRaw(), color, horizAlignment, vertAlignment, minimum);
}

void BGE::FontOpenGLES2::drawString(std::string str, const float *rawMatrix, Color &color, FontHorizontalAlignment horizAlignment, FontVerticalAlignment vertAlignment, bool minimum) {
